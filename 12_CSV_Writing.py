...
with open(output_filename, 'w', encoding='utf-8', newline='') as output_file:
    writer = csv.writer(output_file, quoting=csv.QUOTE_NONNUMERIC)  # quoting=csv.QUOTE_NONNUMERIC quotes non-numbers

    writer.writerow(column_headings)    # .writerow() only writes 1 row

    writer.writerows(cereals)   # .writerows() method with iterate over the row we passed and create csv file
# .writerow() method will iterate 1 row, in that case loop will be used
